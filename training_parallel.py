...
def train_parallel_epochs(p1s, p2s, optimizer, p1net, replay,
	formatid, player_teams,
	alpha, warmup_epochs, train_update_iters,
	epochs, batch_size, parallel_per_batch, eval_epoch_every,
	gamma, lam,
	...
	print(f'\nEpochs: {epochs}\nGames per epoch: {batch_size * parallel_per_batch}\n'
		  f'(batch size: {batch_size}; in parallel: {parallel_per_batch})\n')
	...
	for i in range(epochs):
		...
		for j in range(batch_size):
			winner_strings = run_parallel_learning_episode(
				parallel_per_batch, p1s, p2s, p1net, formatid=formatid, team=player_teams, verbose=False)
		...
		if (i % eval_epoch_every == eval_epoch_every - 1):
			...
			for j in range(batch_size):
				winner_strings = run_parallel_learning_episode(
					parallel_per_batch, p1s, p2s, p1net, verbose=False)
			...
			if(p1winrate_eval >= max_eval_winrate):
				torch.save(
					p1net.state_dict(), 
					'output/' + fstring + '_net_' + str(c) + '_' + str(i) + '.pth')
				
				torch.save(
					v_target_net.state_dict(), 
					'output/' + fstring + '_vtarget_' + str(c) + '_' + str(i) + '.pth')

				torch.save(
					optimizer.state_dict(), 
					'output/' + fstring + '_optim_' + str(c) + '_' + str(i) + '.pth')
...
if __name__ == '__main__':
	...
	p1s, p2s, optimizer, p1net, replay, results = train_parallel_epochs(
		p1s, p2s, optimizer, p1net, replay,
		formatid=formatid, player_teams=player_teams,
		alpha=alpha, warmup_epochs=warmup_epochs, train_update_iters=train_update_iters,
		epochs=epochs, batch_size=batch_size, parallel_per_batch=parallel_per_batch, eval_epoch_every=eval_epoch_every,
		gamma=gamma, lam=lam,
		fstring=fstring, tt_prnt=print_obj_every, verbose=verbose
	)

	with open('output/' + fstring + '_train_win_rates' + str(c) + '.csv', 'w') as myfile:
		wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
		wr.writerow(results['train_win_rates'])

	with open('output/' + fstring + '_eval_win_rates' + str(c) + '.csv', 'w') as myfile:
		wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
