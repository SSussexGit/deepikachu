...
var _fs = require('../../.lib-dist/fs');
 ...
 const commands = {
	...
	async adddatacenters(target, room, user, connection, cmd) {
		...
		const text = await _fs.FS.call(void 0, 'config/datacenters.csv').readIfExists();
		const datacenters = [];
		...
		await _fs.FS.call(void 0, 'config/datacenters.csv').write(output);
